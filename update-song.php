...
if (isset($_POST["updateSong"])) {
    ...
    if ($artistResult->num_rows > 0 && $genreResult->num_rows > 0 && $albumResult->num_rows > 0) {
        // Update the song details in the database
        $updateQuery = "UPDATE songs SET title = '$title', artist = '$artist', album = '$album', genre = '$genre' WHERE id = '$songId'";
        
        if ($con->query($updateQuery) === TRUE) {
            // Song updated successfully, redirect to remove-song.php
            header("Location: remove-son.php?success");
            exit(); // Make sure to exit after redirecting
        } else {
            // Error updating song, redirect to remove-song.php with an error message
            header("Location: remove-son.php?error=1");
            exit(); // Make sure to exit after redirecting
        }
    } else {
        // Invalid artist ID, genre ID, or album ID, redirect to remove-song.php with an error message
        header("Location: remove-son.php?error=2");
        exit(); // Make sure to exit after redirecting
    }
}
?>
